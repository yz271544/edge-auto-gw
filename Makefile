...
.PHONY: images
images:
	docker build --build-arg GO_LDFLAGS=${GO_LDFLAGS} -t ${REPO}kubeedge/edge-auto-gw:${IMAGE_TAG} -f build/Dockerfile .

.PHONY: debugimage
debugimages:
	docker build --build-arg GO_LDFLAGS=${GO_LDFLAGS} -t ${REPO}kubeedge/edge-auto-gw-debug:${IMAGE_TAG} -f build/Dockerfile.debug .

.PHONY: ubiagent
ubiimages:
	docker build --build-arg GO_LDFLAGS=${GO_LDFLAGS} -t ${REPO}kubeedge/edge-auto-gw:${IMAGE_TAG}-ubi8 -f build/Dockerfile-ubi8.docker .
