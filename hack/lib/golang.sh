...
edge_auto_gw::version::get_version_info() {
...
# edge_auto_gw::binaries_from_targets take a list of build targets and return the
# full go package to be built
edge_auto_gw::golang::binaries_from_targets() {
  local target
  for target in "$@"; do
    echo "${EDGE_AUTO_GW_GO_PACKAGE}/${target}"
  done
}

edge_auto_gw::check::env() {
  errors=()
  if [ -z $GOPATH ]; then
...
edge_auto_gw::golang::get_all_targets() {
  local -a targets
  for bt in "${ALL_BINARIES_AND_TARGETS[@]}" ; do
    targets+=("${bt##*:}")
  done
...
edge_auto_gw::golang::get_all_binaries() {
  local -a binaries
  for bt in "${ALL_BINARIES_AND_TARGETS[@]}" ; do
    binaries+=("${bt%%:*}")
  done
...
IFS=" " read -ra EDGE_AUTO_GW_ALL_TARGETS <<< "$(edge_auto_gw::golang::get_all_targets)"
IFS=" " read -ra EDGE_AUTO_GW_ALL_BINARIES<<< "$(edge_auto_gw::golang::get_all_binaries)"

edge_auto_gw::golang::build_binaries() {
  edge_auto_gw::check::env
  local -a targets=()
  local binArg
  for binArg in "$@"; do
    targets+=("$(edge_auto_gw::golang::get_target_by_binary $binArg)")
  done


  if [[ ${#targets[@]} -eq 0 ]]; then
    targets=("${EDGE_AUTO_GW_ALL_TARGETS[@]}")
  fi


  local -a binaries
  while IFS="" read -r binary; do binaries+=("$binary"); done < <(edge_auto_gw::golang::binaries_from_targets "${targets[@]}")
  ...
  goldflags="${GOLDFLAGS=-s -w -buildid=} $(edge_auto_gw::version::ldflags)"
  gogcflags="${GOGCFLAGS:-}"
